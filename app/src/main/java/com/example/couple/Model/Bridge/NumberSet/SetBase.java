...
public class SetBase {
    ...
    public SetBase(int first, int second) {
    ...
    public static SetBase getFrom(int couple) {
        if (couple < 0 || couple > 99) {
            return SetBase.getEmpty();
        }
        return new SetBase(couple / 10, couple % 10);
    }

    public static SetBase getEmpty() {
        return new SetBase(Const.EMPTY_VALUE, Const.EMPTY_VALUE);
    }
    ...
    public boolean equalsSet(SetBase setBase) {
        return CoupleBase.getSmallShadow(getSetInt()) == CoupleBase.getSmallShadow(setBase.getSetInt());
    }
